...
# Tests for OpenCV::Cv::Histogram
class TestHistogram < OpenCVTestCase
  def setup
    @img = IplImage.load(FILENAME_LENA32x32, 0)
    dim = 1
    sizes = [8]
    ranges = [[0, 255]]
    @hist1 = Cv::Histogram.new(dim, sizes, CV_HIST_ARRAY, ranges, true).calc_hist!([@img])
    dim = 2
    sizes = [8, 16]
    ranges = [[0, 255], [0, 255]]
    @hist2 = Cv::Histogram.new(dim, sizes, CV_HIST_SPARSE, ranges, true).calc_hist!([@img, @img])
  end
  ...
  def test_initialize
    ...
    sizes = [256]
    ranges = [[0, 256]]
    hist1 = Cv::Histogram.new(dim, sizes, CV_HIST_ARRAY, ranges, true)
    hist2 = Cv::Histogram.new(dim, sizes, CV_HIST_ARRAY)
    hist3 = Cv::Histogram.new(dim, sizes, CV_HIST_ARRAY, nil)
    hist4 = Cv::Histogram.new(dim, sizes, CV_HIST_ARRAY, nil, false)
  end
  ...
  def test_calc_hist
    ...
    assert_equal(Cv::Histogram, @hist1.calc_hist([img]).class)
    assert_equal(Cv::Histogram, @hist1.calc_hist([img, img]).class)
    assert_equal(Cv::Histogram, @hist1.calc_hist([img], true).class)
    assert_equal(Cv::Histogram, @hist1.calc_hist([img, img], false).class)
    mask = Cv::Mat.new(1, 1, :cv8u, 1)
    assert_equal(Cv::Histogram, @hist1.calc_hist([img], true, mask).class)
  ...
  def test_set_hist_bin_ranges
    dim = 1
    sizes = [8]
    hist = Cv::Histogram.new(dim, sizes, CV_HIST_ARRAY)
