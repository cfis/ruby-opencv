...
class OpenCVTest < OpenCVTestCase
  def test_constants
    # OpenCV version
    ...
    # Retrieval mode
    assert_equal(0, Cv::RetrievalModes::RETR_EXTERNAL_EXTERNAL)
    assert_equal(1, Cv::RetrievalModes::RETR_EXTERNAL_LIST)
    assert_equal(2, Cv::RetrievalModes::RETR_EXTERNAL_CCOMP)
    assert_equal(3, Cv::RetrievalModes::RETR_EXTERNAL_TREE)
  ...
  def test_cvt_color_funcs
    mat_1ch = Cv::Mat.new(1, 1, :cv8u, 1)
    mat_1ch[0] = Cv::Scalar.new(10)
    mat_3ch = Cv::Mat.new(1, 1, :cv8u, 3)
    mat_3ch[0] = Cv::Scalar.new(10, 20, 30)
    mat_4ch = Cv::Mat.new(1, 1, :cv8u, 4)
    mat_4ch[0] = Cv::Scalar.new(10, 20, 30, 40)
    ...
    [mat_3ch.BGR2BGRA, mat_3ch.RGB2RGBA].each { |m|
      ...
      assert_cvscalar_equal(Cv::Scalar.new(10, 20, 30, 255), m[0])
    }
    [mat_3ch.BGR2RGBA, mat_3ch.RGB2BGRA].each { |m|
      assert_equal(Cv::Mat, m.class)
      assert_equal(4, m.channel)
      assert_cvscalar_equal(Cv::Scalar.new(30, 20, 10, 255), m[0])
    }
    [mat_4ch.BGRA2BGR, mat_4ch.RGBA2RGB].each { |m|
      assert_equal(Cv::Mat, m.class)
      assert_equal(3, m.channel)
      assert_cvscalar_equal(Cv::Scalar.new(10, 20, 30, 0), m[0])
    }
    [mat_4ch.RGBA2BGR, mat_4ch.BGRA2RGB].each { |m|
      assert_equal(Cv::Mat, m.class)
      assert_equal(3, m.channel)
      assert_cvscalar_equal(Cv::Scalar.new(30, 20, 10, 0), m[0])
    }
    ...
    [mat_4ch.BGRA2RGBA, mat_4ch.RGBA2BGRA].each { |m|
      assert_equal(Cv::Mat, m.class)
      assert_equal(4, m.channel)
      assert_cvscalar_equal(Cv::Scalar.new(30, 20, 10, 40), m[0])
    }
    # RGB <=> GRAY
    [mat_3ch.BGR2GRAY, mat_4ch.BGRA2GRAY].each { |m|
      assert_equal(Cv::Mat, m.class)
      assert_equal(1, m.channel)
      assert_cvscalar_equal(Cv::Scalar.new(gray_bgr, 0, 0, 0), m[0])
    }
    [mat_3ch.RGB2GRAY, mat_4ch.RGBA2GRAY].each { |m|
      assert_equal(Cv::Mat, m.class)
      assert_equal(1, m.channel)
      assert_cvscalar_equal(Cv::Scalar.new(gray_rgb, 0, 0, 0), m[0])
    }
    [mat_1ch.GRAY2BGR, mat_1ch.GRAY2RGB].each { |m|
      assert_equal(Cv::Mat, m.class)
      assert_equal(3, m.channel)
      assert_cvscalar_equal(Cv::Scalar.new(10, 10, 10, 0), m[0])
    }
    [mat_1ch.GRAY2BGRA, mat_1ch.GRAY2RGBA].each { |m|
      assert_equal(Cv::Mat, m.class)
      assert_equal(4, m.channel)
      assert_cvscalar_equal(Cv::Scalar.new(10, 10, 10, 255), m[0])
    }
