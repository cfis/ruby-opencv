...
class MatTest < OpenCVTestCase
  ...
  def test_ptr_uchar
    ...
    row = mat.ptr(0).to_array
    assert_equal([1, 0, 0], row)

    row = mat.ptr(1).to_array(0, 3)
    assert_equal([0, 1, 0], row)

    row = mat.ptr(2).to_array
  ...
  def test_data
    ...
    assert_kind_of(Rice::PointerView≺unsigned char≻, mat.data)
    data = mat.data.buffer(0, size)
    assert_equal("\x01\x01\x02\x02\x03\x03\x04\x04", data)

    assert_kind_of(Array, mat.data.to_array)
    array = mat.data.to_array(0, size)
  ...
  def test_slice_range_pointer
    ...
    row_range = Cv::Range.new(0, 3)
    col_range = Cv::Range.new(0, 3)
    ranges = [row_range, col_range]
    mat_slice = mat[ranges]
    assert_equal("[1, 2, 3;\n 4, 5, 6;\n 7, 8, 9]", mat_slice.inspect)
  ...
  def test_bitwise_and
    ...
    mat3 = mat1.bitwise_and(mat2, mask)
    assert_equal(mat1.rows, mat3.rows)
    assert_equal(mat1.cols, mat3.cols)
    ...
    mat3 = mat1.bitwise_and(Cv::Scalar.new(1, 2, 3, 4), mask)
    assert_equal(mat1.rows, mat3.rows)
    assert_equal(mat1.cols, mat3.cols)
    assert_each_element(mat3) do |row, col, count|
      n = count + 1
  ...
  def test_bitwise_or
    ...
    # Cv::Mat | Cv::Mat with mask
    mat3 = mat1.bitwise_or(mat2, mask)
    assert_equal(mat1.rows, mat3.rows)
    assert_equal(mat1.cols, mat3.cols)
    assert_each_element(mat3) do |row, col, count|
      n = count + 1
    ...
    # Cv::Mat | Cv::Scalar with mask
    mat4 = mat1.bitwise_or(Cv::Scalar.new(1, 2, 3, 4), mask)
    assert_equal(mat1.rows, mat4.rows)
    assert_equal(mat1.cols, mat4.cols)
    assert_each_element(mat4) do |row, col, count|
      n = count + 1
  ...
  def test_bitwise_xor
    ...
    # Cv::Mat ^ Cv::Mat with mask
    mat3 = mat1.bitwise_xor(mat2, mask)
    assert_equal(mat1.rows, mat3.rows)
    assert_equal(mat1.cols, mat3.cols)
    assert_each_element(mat3) do |row, col, count|
      n = count + 1
    ...
    # Cv::Mat ^ Cv::Scalar with mask
    mat3 = mat1.bitwise_xor(Cv::Scalar.new(1, 2, 3, 4), mask)
    assert_equal(mat1.rows, mat3.rows)
    assert_equal(mat1.cols, mat3.cols)
    assert_each_element(mat3) do |row, col, count|
      n = count + 1
  ...
  def test_invert
    ...
    mat2 = mat1.invert
    mat3 = mat1.invert(Cv::DecompTypes::DECOMP_LU)
    mat4 = mat1.invert(Cv::DecompTypes::DECOMP_SVD)
    mat5 = mat1.invert(Cv::DecompTypes::DECOMP_CHOLESKY)
    ...
    mat6 = mat1.invert(Cv::DecompTypes::DECOMP_EIG)
