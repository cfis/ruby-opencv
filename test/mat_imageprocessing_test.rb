...
# Tests for image processing functions of OpenCV::Cv::Mat
class Mat_imageprocessingTest < OpenCVTestCase
  ...
  def test_sobel
    mat0 = Cv::Mat.load(FILENAME_LENA256x256, CV_LOAD_IMAGE_GRAYSCALE)
    ...
    assert_equal(:cv16s, Cv::Mat.new(16, 16, :cv8u, 1).sobel(1, 1).depth)
    assert_equal(:cv32f, Cv::Mat.new(16, 16, :cv32f, 1).sobel(1, 1).depth)
    (DEPTH.keys - [:cv8u, :cv32f]).each { |depth|
      assert_raise(ArgumentError) {
        Cv::Mat.new(3, 3, depth).sobel(1, 1)
      }
    }
  ...
  def test_laplace
    ...
    assert_equal(:cv16s, Cv::Mat.new(16, 16, :cv8u, 1).laplace.depth)
    assert_equal(:cv32f, Cv::Mat.new(16, 16, :cv32f, 1).laplace.depth)
    (DEPTH.keys - [:cv8u, :cv32f]).each { |depth|
      assert_raise(ArgumentError) {
        Cv::Mat.new(3, 3, depth).laplace
      }
    }
  ...
  def test_corner_eigenvv
    ...
    flunk('FIXME: Cv::Mat#corner_eigenvv is not tested yet.')
  end
  ...
  def test_corner_min_eigen_val
    ...
    flunk('FIXME: Cv::Mat#corner_min_eigen_val is not tested yet.')
  end
  ...
  def test_find_chessboard_corners
    mat = Cv::Mat.load(FILENAME_CHESSBOARD, CV_LOAD_IMAGE_GRAYSCALE)
  ...
  def test_good_features_to_track
    mat0 = Cv::Mat.load(FILENAME_LENA32x32, CV_LOAD_IMAGE_GRAYSCALE)
  ...
  def test_rect_sub_pix
    mat0 = Cv::Mat.load(FILENAME_LENA256x256, CV_LOAD_IMAGE_ANYCOLOR | CV_LOAD_IMAGE_ANYDEPTH)
  ...
  def test_log_polar
    mat0 = Cv::Mat.load(FILENAME_FRUITS, CV_LOAD_IMAGE_ANYCOLOR | CV_LOAD_IMAGE_ANYDEPTH)
  ...
  def test_copy_make_border
    mat0 = create_cvmat(32, 32, :cv8u, 1) { Cv::Scalar.new(128) }
  ...
  def test_integral
    mat0 = create_cvmat(3, 3, :cv8u, 1) { |j, i, n| Cv::Scalar.new(n) }
    result_sum = []
    result_sqsum = []
    result_tiled_sum = []
  ...
  def test_adaptive_threshold
    ...
    [mat2a, mat2b].each { |mat2|
      assert_equal(Cv::Mat, mat2.class)
    ...
    [mat3a, mat3b].each { |mat3|
      assert_equal(Cv::Mat, mat3.class)
  ...
  def test_hough_lines
    mat0 = Cv::Mat.load(FILENAME_LINES, CV_LOAD_IMAGE_ANYCOLOR | CV_LOAD_IMAGE_ANYDEPTH)
    # make a binary image
    mat = Cv::Mat.new(mat0.rows, mat0.cols, :cv8u, 1)
  ...
  def test_inpaint
    mat = Cv::Mat.load(FILENAME_LENA_INPAINT, CV_LOAD_IMAGE_ANYCOLOR | CV_LOAD_IMAGE_ANYDEPTH)
    mask = Cv::Mat.load(FILENAME_INPAINT_MASK, CV_LOAD_IMAGE_GRAYSCALE)
    ...
    assert_raise(Cv::StsUnsupportedFormat) {
      Cv::Mat.new(10, 10, :cv32f, 3).inpaint(:ns, Cv::Mat.new(10, 10, :cv8u, 1), 10)
    }
  end

  def test_equalize_hist
    mat = Cv::Mat.load(FILENAME_LENA256x256, CV_LOAD_IMAGE_GRAYSCALE)
  ...
  def test_match_template
    mat = Cv::Mat.load(FILENAME_LENA256x256, CV_LOAD_IMAGE_ANYCOLOR | CV_LOAD_IMAGE_ANYDEPTH)
    templ = Cv::Mat.load(FILENAME_LENA_EYES, CV_LOAD_IMAGE_ANYCOLOR | CV_LOAD_IMAGE_ANYDEPTH)
  ...
  def test_match_shapes
    mat_cv = Cv::Mat.load(FILENAME_STR_CV, CV_LOAD_IMAGE_GRAYSCALE)
    mat_ov = Cv::Mat.load(FILENAME_STR_OV, CV_LOAD_IMAGE_GRAYSCALE)
    mat_cv_rotated = Cv::Mat.load(FILENAME_STR_CV_ROTATED, CV_LOAD_IMAGE_GRAYSCALE)
  ...
  def test_snake_image
    ...
    mat = Cv::Mat.new(center.y * 2, center.x * 2, :cv8u, 1).zero!
