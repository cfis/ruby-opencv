...
void Init_Utility()
{
  ...
  rb_mCvCudaDevice.define_module_function<bool(*)(const int[2][2], const int[2], int[2])>("solve2x2?", &cv::cuda::device::solve2x2,
    Arg("a") = 22, Arg("b") = 2, Arg("x") = 2);
  
  rb_mCvCudaDevice.define_module_function<bool(*)(const int[3][3], const int[3], int[3])>("solve3x3?", &cv::cuda::device::solve3x3,
    Arg("a") = 33, Arg("b") = 3, Arg("x") = 3);
