...
void Init_Instrumentation()
{
  ...
  Class rb_cCvInstrNodeData = define_class_under<cv::instr::NodeData>(rb_mCvInstr, "NodeData").
    define_constructor(Constructor<cv::instr::NodeData, const char*, const char*, int, void*, bool, cv::instr::TYPE, cv::instr::IMPL>(),
      Arg("fun_name") = static_cast<const char *>(0), Arg("file_name") = static_cast<const char *>(NULL), Arg("line_num") = static_cast<int>(0), Arg("ret_address") = static_cast<void *>(NULL), Arg("always_expand") = static_cast<bool>(false), Arg("instr_type") = static_cast<cv::instr::TYPE>(TYPE_GENERAL), Arg("impl_type") = static_cast<cv::instr::IMPL>(IMPL_PLAIN)).
