...
void Init_Optim()
{
  ...
  Class rb_cCvDownhillSolver = define_class_under<cv::DownhillSolver, cv::MinProblemSolver>(rb_mCv, "DownhillSolver").
    ...
    define_singleton_function("create", &cv::DownhillSolver::create,
      Arg("f") = static_cast<const Ptr<MinProblemSolver::Function> &>(Ptr<MinProblemSolver::Function>()), Arg("init_step") = static_cast<InputArray>(Mat_<double>(1,1,0.0)), Arg("termcrit") = static_cast<TermCriteria>(TermCriteria(TermCriteria::MAX_ITER+TermCriteria::EPS,5000,0.000001)));
  ...
  Class rb_cCvConjGradSolver = define_class_under<cv::ConjGradSolver, cv::MinProblemSolver>(rb_mCv, "ConjGradSolver").
    define_singleton_function("create", &cv::ConjGradSolver::create,
      Arg("f") = static_cast<const Ptr<MinProblemSolver::Function> &>(Ptr<ConjGradSolver::Function>()), Arg("termcrit") = static_cast<TermCriteria>(TermCriteria(TermCriteria::MAX_ITER+TermCriteria::EPS,5000,0.000001)));
