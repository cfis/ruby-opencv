...
void Init_Stitching()
{
  ...
  Class rb_cCvStitcher = define_class_under<cv::Stitcher>(rb_mCv, "Stitcher").
    ...
    define_singleton_function("create", &cv::Stitcher::create,
      Arg("mode") = static_cast<Mode>(Stitcher::PANORAMA)).
    ...
    define_method("estimate_transform", &cv::Stitcher::estimateTransform,
      Arg("images"), Arg("masks") = static_cast<InputArrayOfArrays>(noArray())).
  ...
  rb_mCv.define_module_function("create_stitcher", &cv::createStitcher,
    Arg("try_use_gpu") = static_cast<bool>(false));
  
  rb_mCv.define_module_function("create_stitcher_scans", &cv::createStitcherScans,
    Arg("try_use_gpu") = static_cast<bool>(false));
