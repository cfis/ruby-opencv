...
void Init_Imgcodecs()
{
  ...
  rb_mCv.define_module_function<bool(*)(cv::InputArray, int, std::vector<cv::Mat>&, const cv::Range&)>("imdecodemulti?", &cv::imdecodemulti,
    Arg("buf"), Arg("flags"), Arg("mats"), Arg("range"));
  
  rb_mCv.define_module_function<bool(*)(const cv::String&, cv::InputArray, std::vector<unsigned char>&, const std::vector<int>&)>("imencode?", &cv::imencode,
    Arg("ext"), Arg("img"), Arg("buf"), Arg("params"));
...
//    define_method<cv::Ptr<cv::ImageCollection::Impl>(cv::ImageCollection::*)()>("get_impl", &cv::ImageCollection::getImpl);
  ...
  Class rb_cCvImageCollectionIterator = define_class_under<cv::ImageCollection::iterator>(rb_cCvImageCollection, "Iterator").
    ...
    define_constructor(Constructor<cv::ImageCollection::iterator, cv::ImageCollection*, int>(),
      Arg("col"), Arg("end")).
    define_method<cv::Mat&(cv::ImageCollection::iterator::*)()>("*", &cv::ImageCollection::iterator::operator*).
    define_method<cv::Mat*(cv::ImageCollection::iterator::*)()>("->", &cv::ImageCollection::iterator::operator->).
    define_method<cv::ImageCollection::iterator&(cv::ImageCollection::iterator::*)()>("++", &cv::ImageCollection::iterator::operator++).
    define_method<cv::ImageCollection::iterator(cv::ImageCollection::iterator::*)(int)>("++", &cv::ImageCollection::iterator::operator++,
      Arg(""));
