...
void Init_ArucoDetector()
{
  ...
  Class rb_cCvArucoArucoDetector = define_class_under<cv::aruco::ArucoDetector, cv::Algorithm>(rb_mCvAruco, "ArucoDetector").
    define_constructor(Constructor<cv::aruco::ArucoDetector, const cv::aruco::Dictionary&, const cv::aruco::DetectorParameters&, const cv::aruco::RefineParameters&>(),
      Arg("dictionary") = static_cast<const Dictionary &>(getPredefinedDictionary(cv::aruco::DICT_4X4_50)), Arg("detector_params") = static_cast<const DetectorParameters &>(DetectorParameters()), Arg("refine_params") = static_cast<const RefineParameters &>(RefineParameters())).
    define_method("detect_markers", &cv::aruco::ArucoDetector::detectMarkers,
      Arg("image"), Arg("corners"), Arg("ids"), Arg("rejected_img_points") = static_cast<OutputArrayOfArrays>(noArray())).
    define_method("refine_detected_markers", &cv::aruco::ArucoDetector::refineDetectedMarkers,
      Arg("image"), Arg("board"), Arg("detected_corners"), Arg("detected_ids"), Arg("rejected_corners"), Arg("camera_matrix") = static_cast<InputArray>(noArray()), Arg("dist_coeffs") = static_cast<InputArray>(noArray()), Arg("recovered_idxs") = static_cast<OutputArray>(noArray())).
  ...
  rb_mCvAruco.define_module_function("draw_detected_markers", &cv::aruco::drawDetectedMarkers,
    Arg("image"), Arg("corners"), Arg("ids") = static_cast<InputArray>(noArray()), Arg("border_color") = static_cast<Scalar>(Scalar(0, 255, 0)));
