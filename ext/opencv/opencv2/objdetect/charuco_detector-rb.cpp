...
void Init_CharucoDetector()
{
  ...
  Class rb_cCvArucoCharucoDetector = define_class_under<cv::aruco::CharucoDetector, cv::Algorithm>(rb_mCvAruco, "CharucoDetector").
    define_constructor(Constructor<cv::aruco::CharucoDetector, const cv::aruco::CharucoBoard&, const cv::aruco::CharucoParameters&, const cv::aruco::DetectorParameters&, const cv::aruco::RefineParameters&>(),
      Arg("board"), Arg("charuco_params") = static_cast<const CharucoParameters &>(CharucoParameters()), Arg("detector_params") = static_cast<const DetectorParameters &>(DetectorParameters()), Arg("refine_params") = static_cast<const RefineParameters &>(RefineParameters())).
    ...
    define_method("detect_board", &cv::aruco::CharucoDetector::detectBoard,
      Arg("image"), Arg("charuco_corners"), Arg("charuco_ids"), Arg("marker_corners") = static_cast<InputOutputArrayOfArrays>(noArray()), Arg("marker_ids") = static_cast<InputOutputArray>(noArray())).
    define_method("detect_diamonds", &cv::aruco::CharucoDetector::detectDiamonds,
      Arg("image"), Arg("diamond_corners"), Arg("diamond_ids"), Arg("marker_corners") = static_cast<InputOutputArrayOfArrays>(noArray()), Arg("marker_ids") = static_cast<InputOutputArray>(noArray()));
  
  
  rb_mCvAruco.define_module_function("draw_detected_corners_charuco", &cv::aruco::drawDetectedCornersCharuco,
    Arg("image"), Arg("charuco_corners"), Arg("charuco_ids") = static_cast<InputArray>(noArray()), Arg("corner_color") = static_cast<Scalar>(Scalar(255, 0, 0)));
  
  rb_mCvAruco.define_module_function("draw_detected_diamonds", &cv::aruco::drawDetectedDiamonds,
    Arg("image"), Arg("diamond_corners"), Arg("diamond_ids") = static_cast<InputArray>(noArray()), Arg("border_color") = static_cast<Scalar>(Scalar(0, 0, 255)));
